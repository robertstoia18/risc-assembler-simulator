...
public class BasicPipelineController
{
    public BasicPipelineState State { get; }
    ...
    public BasicPipelineController(BasicPipelineState state)
    {
        State = state;
        for (int i = 0; i < 5; i++)
    ...
    private void SaveDisplayState()
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            DisplaySlots[i] = new PipelineSlot
            {
                ...
                A = src.A, B = src.B, C = src.C, MAR = src.MAR,
                IsStall = src.IsStall,
    ...
    private void StageWB()
    {
        ...
        if (instr.IsHalt) { Halted = true; return; }
    ...
    private int ResolveOperand(int reg, Instruction instr)
    {
        ...
        {
            LastForwardings.Add(new ForwardingEvent { FromStage = (int)PipelineStage.EX, ToStage = (int)PipelineStage.OF, Register = reg, Value = _fwdFromEx.Value.Val });
            return _fwdFromEx.Value.Val;
        }

        if (_fwdFromMem.HasValue && _fwdFromMem.Value.Reg == reg)
        {
            LastForwardings.Add(new ForwardingEvent { FromStage = (int)PipelineStage.MEM, ToStage = (int)PipelineStage.OF, Register = reg, Value = _fwdFromMem.Value.Val });
    ...
    private void StageMEM()
    {
        ...
        if (instr.IsLoad) { slot.C = State.Memory.Read(slot.MAR); State.MDR = slot.C; }
        else if (instr.IsStore) { State.Memory.Write(slot.MAR, slot.C); State.MDR = slot.C; }
    }
    ...
    private void StageEX()
    {
        ...
        if (instr.IsLoad) { slot.MAR = a + instr.Immediate; State.MAR = slot.MAR; return; }
        if (instr.IsStore) { slot.MAR = a + instr.Immediate; State.MAR = slot.MAR; return; }

        if (instr.IsJump)
        {
            if (instr.SourceMode == AddressingMode.AM) State.PC = instr.Immediate;
            else if (instr.SourceMode == AddressingMode.AI) State.PC = slot.A;
            else if (instr.SourceMode == AddressingMode.AX) State.PC = slot.A + instr.Immediate;

            if (instr.Op == Opcode.JAL)
                slot.C = instr.OriginAddress + InstructionSet.GetWordCount(instr);

            FlushIF(); FlushOF(); return;
        }
        ...
        if (instr.IsBranch)
        {
            ...
            if (condition) { State.PC = target; FlushIF(); FlushOF(); }
            return;
        }

        slot.C = AluOp(instr.Op, a, slot.B, instr.Immediate, instr.SourceMode);
        State.C = slot.C;
    }
    ...
    private void StageIF()
    {
        ...
        if (!_instrByAddress.TryGetValue(pc, out var instr))
        {
            ...
            State.IF.A = 0; State.IF.B = 0; State.IF.C = 0; State.IF.IsStall = false;
            return;
        }
        ...
        State.IF.A = 0; State.IF.B = 0; State.IF.C = 0;
        State.IF.IsStall = false; State.IF.BranchTaken = false;
    }
    ...
    private void AdvancePipeline(bool stall)
    {
        if (stall)
        {
            ...
            State.Slots[(int)PipelineStage.EX] = new PipelineSlot { Instruction = Instruction.MakeNop(), IsStall = true };
        }
        else
        {
            State.Slots[(int)PipelineStage.WB] = State.Slots[(int)PipelineStage.MEM];
    ...
    {
        State.Slots[(int)PipelineStage.IF] = new PipelineSlot { Instruction = Instruction.MakeNop(), IsStall = true };
    }

    private void FlushOF()
    {
        State.Slots[(int)PipelineStage.OF] = new PipelineSlot { Instruction = Instruction.MakeNop(), IsStall = true };
    }
    ...
    private bool EvalBranchCondition(Opcode op, int a, int b) => op switch
    {
        Opcode.BEQ => a == b,
        Opcode.BNE => a != b,
        Opcode.BL => a < b,
